...
<body>
    ...
    <div class="row">
    <div class="col-lg-4 offset-md-4 mt-4">
        <form action="" class="text-center font-italic">
                <input id="myInput" type="text" name="search" placeholder="Masukkan Nomor Polisi atau Nomor Kendaraan" 
                    class="rounded w-100 text-white pl-2 text-center" onkeyup="cari()"
                    style="background-color:rgb(0,0,0,0.8);border:none; height=40px">
        </form>
    </div>
            ...
            <table class="table table-dark text-center" id="tableSparepart">
                <thead>
                    <tr>
                        ...
                        <th scope="col">Nomor Telepon Customer</th>
...
<script>
    let col = ['no_transaksi', 'no_polisi', 'no_telp_customer', 'status_transaksi'];   
    ...
    axios.get('api/penjualan/tampilriwayat', config)
    .then((result) => {
        console.log(result.data);
        let penjualan = result.data;    
        for(let i = 0; i < penjualan.length; i++) {
            let tr = tableSparepart.insertRow(-1);
            let td = document.createElement('td');      
            for(j = 0; j <= col.length-1; j++){
                td = tr.insertCell();  
                td.innerHTML = penjualan[i][col[j]];
            }  
        }
    }).catch((err) => {
    ...
    function cari() {
        var input, sfilter, table, tr, td,td2 , i, txtValue;
