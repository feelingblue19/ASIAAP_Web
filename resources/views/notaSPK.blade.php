...
<body>
    <div id="fullcontainer">
        <div id="container">
		<img src="logoAA.ico" height="170px" width="130" align="left"/>
            <div id="header">
                <p> A T M A - A U T O </p>
...
<script>
    ...
    .then((result) => {
        let merk = result.data[0].nama_merk;
        let tipe = result.data[0].nama_tipe;
        let nopol = result.data[0].no_polisi;
        document.querySelector('#motor').innerHTML = merk + ' ' + tipe + ' ' + nopol;
    ...
    function sparepart() {
        axios.get('api/getpenjualansprsatu/' + notrans + '/' + kendaraan)
    .then((result) => {
		let tableSparepart = document.querySelector('#sparepart tbody');
        for(let i = 0; i < result.data.length; i++) {
            ...
            for(j = 0; j < 5; j++){
                ...
                if(j == 0) {
                    ...
                    td.innerHTML = result.data[i].sparepart.penempatan_sparepart;
                } else if (j==4) {
    ...
    .then((result) => {
		...
		for(let i = 0; i < result.data.length; i++) {
            ...
            for(j = 0; j < 3; j++){
                td = tr.insertCell();  
                if(j == 0) {
					td.innerHTML = result.data[i].jasa_service.id_jasa_service;
                }else if (j == 1) {
                    td.innerHTML = result.data[i].jasa_service.nama_jasa_service;
                } else if (j == 2) {
                    td.innerHTML = result.data[i].jumlah_penjualan_jasa;
                } 
            }
		}
