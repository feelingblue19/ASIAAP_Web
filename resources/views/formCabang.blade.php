...
<head>
    ...
    <title>Atma Auto: Form Cabang</title>
</head>
<body>
    ...
    <div class="container-fluid" style="background-color:#34495e">
        ...
        <div class="row">
            ...
            <div class="col-1" style="margin-left: 50px;">
                ...
                <h6 class="text-center" style="color:white">Cabang</h6>
            </div>
            <div class="col-1">
                <a href="merk">
                    <img id="img2" src="OWMerk.png" alt="Menu Merk" class="img-fluid gambarMenu p-3"
                    title="Kelola Data Merk" onmouseover="fadein(2)" onmouseout="fadeout(2)" style="opacity:0.5">
                </a>
    ...
    <br><br>
    <h1 class="text-center font-italic mb-3 text-white">Kelola Data Cabang</h1>
        ...
        <div class="row">
            ...
            <div class="col-4 text-center mb-5 p-4 isi">
                <form onsubmit = "return post()">
                    <h5>Nama Cabang</h5>
                    <input type="text" name="nama" id="nama" placeholder="Masukkan Nama Cabang"
    ...
    <div class="container">
        <div class="row">
            <div class="col-1"></div>
            <div class="col-10">
            <h1 class="text-center font-italic text-white">Cari Data Cabang</h1>
            <form action="" class="text-center font-italic">
                <input id="myInput" type="text" name="search" placeholder="Cari Cabang" 
                class="rounded w-100 text-white pl-2" onkeyup="cari()" 
                style="background-color:rgb(0,0,0,0.8);border:none">
            </form>
            <br>
            <br>
            <h1 class="text-center font-italic text-white">Table Cabang</h1>
            <table class="table table-dark text-center" id="tableCabang">
                <tr>
...
<script>
    ...
    let col = ['id_cabang', 'nama_cabang', 'alamat_cabang'];
    let edited = false;
    let editedID = '';
    let editedPK = '';
    ...
    axios.get('api/cabang/tampil', config)
        ...
        function post() {
            if(document.getElementById("nama").value == "" || document.getElementById("alamat").value == "")
            {
                ...
                if(edited == false) {
                    axios.post('api/cabang/tambah', formData, config)
                    .then((result) => {
                        console.log(result);
                        let cab = {};
                        cab.id_cabang = result.data.cabang.id_cabang;
                        cab.nama_cabang = nama;
                        cab.alamat_cabang = alamat;
                        cabang.push(cab);

                        let tr = tableCabang.insertRow(-1);
                        let td = document.createElement('td'); 
                        for(j = 0; j <= Object.keys(cab).length; j++){
                            td = tr.insertCell();  
                            if(j == Object.keys(cab).length) {
                                let buttonEdit = document.createElement('input');
                                buttonEdit.setAttribute('type', 'button');
                                buttonEdit.setAttribute('value', 'Ubah');
                                buttonEdit.setAttribute('class', 'btn btn-primary');
                    ...
                    axios.post('api/cabang/ubah/' + editedPK, formData, config)
                    .then((result) => {
                        console.log(result);
                        tableCabang.rows[editedID].cells[1].innerHTML = nama;
                        tableCabang.rows[editedID].cells[2].innerHTML = alamat;
                        edited = false;
                        alert("Edit Berhasil!");
                    }).catch((err) => {
                        console.log(err);
        ...
        function hapus(obj) {
            ...
            {
                axios.delete('api/cabang/hapus/'+ obj.parentNode.parentNode.cells[0].innerHTML ,config)
                .then((result) => {
