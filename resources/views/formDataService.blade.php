...
<head>
    ...
    <title>Atma Auto: Form Jasa Service</title>
</head>
<body>
{{-- Header --}}
        ...
        <div class="row">
            ...
            <div class="col-1">
                <a href="service">
                    ...
                    title="Kelola Data Jasa Service">
                </a>
                <h6 id="text3" class="text-center" style="color:white">Jasa</h6>
            </div>
            <div class="col-1">
                <a href="pegawai">
    ...
    <br><br>
    <h1 class="text-center font-italic mb-3 text-white">Kelola Data Jasa Service</h1>
        ...
        <div class="row">
            ...
            <div class="col-4 text-center mb-5 p-4 isi">
                <form onsubmit = "return post()">
                    <h5>Nama Jasa Service</h5>
                    <input type="text" name="nama" id="nama" placeholder="Masukkan Nama Jasa Service"
    ...
    <div class="container">
        <div class="row">
            <div class="col-1"></div>
            <div class="col-10">
            <h1 class="text-center font-italic text-white">Cari Data Jasa Service</h1>
            <form action="" class="text-center font-italic">
                <input id="myInput" type="text" name="search" placeholder="Cari Jasa Service"
                class="rounded w-100 text-white pl-2" onkeyup="cari()"
                style="background-color:rgb(0,0,0,0.8);border:none">
            </form>
            <br>
            <br>
            <h1 class="text-center font-italic text-white">Table Jasa Service</h1>
            <table id="tableService" class="table table-dark text-center">
                <tr>
...
<script>
    ...
    let col = ['id_jasa_service', 'nama_jasa_service', 'harga_jasa_service'];
    let edited = false;
    let editedID = '';
    let editedPK = '';
    ...
    axios.get('api/jasaservice/tampil', config)
        ...
        function post() {
            if(document.getElementById("nama").value == "" || document.getElementById("harga").value == "")
            {
                ...
                let nama_jasa_service = document.querySelector('#nama').value;
                let harga_jasa_service = document.querySelector('#harga').value;
                let formData = new FormData();
                formData.append('nama_jasa_service', nama_jasa_service);
                formData.append('harga_jasa_service', harga_jasa_service);
                if(edited == false) {
                    axios.post('api/jasaservice/tambah', formData, config)
                    .then((result) => {
                        console.log(result);
                        let ser = {};
                        ser.nama_jasa_service = nama_jasa_service;
                        ser.harga_jasa_service = harga_jasa_service;
                        ser.id_jasa_service = result.data.jasa_service.id_jasa_service;
                        service.push(ser);

                        let tr = tableService.insertRow(-1);
                        let td = document.createElement('td');
                        for(j = 0; j <= Object.keys(ser).length; j++){
                            td = tr.insertCell();
                            if(j == Object.keys(ser).length) {
                                let buttonEdit = document.createElement('input');
                                buttonEdit.setAttribute('type', 'button');
                                buttonEdit.setAttribute('value', 'Ubah');
                                buttonEdit.setAttribute('class', 'btn btn-primary');
                    ...
                    axios.post('api/jasaservice/ubah/' + editedPK, formData, config)
                    .then((result) => {
                        tableService.rows[editedID].cells[1].innerHTML = nama_jasa_service;
                        tableService.rows[editedID].cells[2].innerHTML = harga_jasa_service;
                        edited = false;
                        alert("Edit Berhasil!");
                    }).catch((err) => {
                        console.log(err.response);
        ...
        function hapus(obj) {
            ...
            {
                axios.delete('api/jasaservice/hapus/'+ obj.parentNode.parentNode.cells[0].innerHTML ,config)
                .then((result) => {
                    service.splice(obj.parentNode.parentNode.rowIndex-1, 1);
                    tableService.deleteRow(obj.parentNode.parentNode.rowIndex);
        ...
        function ubah(obj) {
            ...
            document.querySelector('#harga').value = obj.parentNode.parentNode.cells[2].innerHTML;
            editedID = obj.parentNode.parentNode.rowIndex;
            editedPK = obj.parentNode.parentNode.cells[0].innerHTML;
            console.log(editedPK);
            edited = true;
        }
